...
class AdminServiceTest {
    ...
    @Test
    @DisplayName("Get Dashboard — handles null orders response gracefully")
    void getDashboard_nullOrdersResponse_handlesGracefully() {
        when(orderFeignClient.getAllOrders(0, 1000))
                .thenReturn(null);
        ...
        assertThat(response.isSuccess()).isTrue();
        assertThat(response.getData().getTotalOrders()).isZero();
    }

    @Test
    @DisplayName("Get Dashboard — handles order with null status gracefully")
    void getDashboard_orderWithNullStatus_handlesGracefully() {
        testOrder.setStatus(null);

        PagedResponse<OrderResponse> ordersWithNullStatus = new PagedResponse<>(
                List.of(testOrder), 0, 10, 1L, 1, true);

        when(orderFeignClient.getAllOrders(0, 1000))
                .thenReturn(ApiResponse.success(ordersWithNullStatus));
        when(authFeignClient.getAllUsers(0, 1))
                .thenReturn(ApiResponse.success(pagedUsers));
    ...
    @Test
    @DisplayName("Get Dashboard — handles order with null total amount")
    void getDashboard_orderWithNullTotalAmount_handlesGracefully() {
        testOrder.setTotalAmount(null);

        PagedResponse<OrderResponse> ordersResponse = new PagedResponse<>(
                List.of(testOrder), 0, 10, 1L, 1, true);

        PagedResponse<UserDTO> usersResponse = new PagedResponse<>(
                List.of(testUser), 0, 10, 1L, 1, true);

        when(orderFeignClient.getAllOrders(0, 1000))
                .thenReturn(ApiResponse.success(ordersResponse));
        when(authFeignClient.getAllUsers(0, 1))
                .thenReturn(ApiResponse.success(usersResponse));

        ApiResponse<DashboardDTO> response = adminService.getDashboard();

        assertThat(response.isSuccess()).isTrue();
        assertThat(response.getData().getTotalRevenue()).isZero();
    }

    @Test
    @DisplayName("Get Dashboard — counts all revenue-generating statuses correctly")
    void getDashboard_allRevenueStatuses_countsCorrectly() {
        OrderResponse paidOrder = new OrderResponse();
        paidOrder.setId(1L);
        paidOrder.setStatus(OrderStatus.PAID);
        paidOrder.setTotalAmount(new BigDecimal("100.00"));

        OrderResponse packedOrder = new OrderResponse();
        packedOrder.setId(2L);
        packedOrder.setStatus(OrderStatus.PACKED);
        packedOrder.setTotalAmount(new BigDecimal("200.00"));

        OrderResponse shippedOrder = new OrderResponse();
        shippedOrder.setId(3L);
        shippedOrder.setStatus(OrderStatus.SHIPPED);
        shippedOrder.setTotalAmount(new BigDecimal("300.00"));

        OrderResponse deliveredOrder = new OrderResponse();
        deliveredOrder.setId(4L);
        deliveredOrder.setStatus(OrderStatus.DELIVERED);
        deliveredOrder.setTotalAmount(new BigDecimal("400.00"));

        PagedResponse<OrderResponse> ordersResponse = new PagedResponse<>(
                List.of(paidOrder, packedOrder, shippedOrder, deliveredOrder),
                0, 10, 4L, 1, true);
        ...
        assertThat(response.getData().getPendingOrders()).isEqualTo(2L);
    }

    @Test
    @DisplayName("Get Dashboard — handles cancelled orders correctly")
    void getDashboard_cancelledOrders_countsCorrectly() {
        ...
        assertThat(response.isSuccess()).isTrue();
        assertThat(response.getData().getCancelledOrders()).isEqualTo(1L);
        assertThat(response.getData().getTotalRevenue()).isZero();
    }
}